...
* 引数を省略 ... 全チャンネルの割り当てを一覧表示します(OPMDRV3.X専用)。
* 引数が1個(チャンネル番号) ... 現在の割り当てを表示します(OPMDRV3.X専用)。
* 引数が2個(チャンネル番号、トラック番号) ... チャンネルにトラックを割り当てます。

ただし、`m_assign -1 -1`として実行すると全チャンネルの割り当てを初期化します
(OPMDRV3.X専用)。
...
`OPM _M_MDREG`でMIDIコントローラー(YM3802)のレジスタに書き込み、結果を表示します。
コマンドライン引数でグループ番号、アドレス番号、書き込むデータを指定します。

> [!WARNING]
> `OPM _M_MDREG`の不具合により、意図しないレジスタへの書き込みや、
> バスエラーやほかの拡張ボードの誤動作が起きます。
...
OPMコールの処理アドレスを表示します。
コマンドライン引数でOPMコール番号を指定します(省略時はすべてのアドレスを表示)。
