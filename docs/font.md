...
## fntadr
コマンドライン引数で指定した文字(1文字)のフォントアドレスを`IOCS _FNTADR`で取得し、
結果を表示します。
以下のオプションが指定できます。
...
* 外字ではありませんが、ついでに以下のものも受け付けます。
  * `0` ... 0xf000～0xf0ff 上付き1/4角文字(カタカナ)
  * `1` ... 0xf100～0xf1ff 上付き1/4角文字(ひらがな)
  * `2` ... 0xf200～0xf2ff 下付き1/4角文字(カタカナ)
  * `3` ... 0xf300～0xf3ff 下付き1/4角文字(ひらがな)
  * `8` ... 0x8000～0x80ff 半角文字(ひらがな)


## uskcg_hex
ユーザー定義外字(0x869f～0x879e、0xeb9f～0xec9e)のフォントを、
文字コードを16進数で表記したものに書き換えます。

半角外字は[uskhw_hex](#uskhw_hex)で書き換えることができます。


## uskcg_save
ユーザー定義外字のすべての文字のフォントを、コマンドライン引数で指定したファイルに
USKCG.SYS形式で保存します。

パターンが定義されていない空白の文字や、USKCGM.Xで扱えない外字A(0x869f～0x879e)
も保存します。

ファイルサイズは75460バイトになりますが、CONFIG.SYSの`USKCG=`行で指定すると
Human68kの不具合により最後の文字のパターンが正しく設定されません。
...
ユーザー定義外字(全角)は[uskcg_hex](#uskcg_hex)で書き換えることができます。
