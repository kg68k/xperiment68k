...
## si_pcgexp
PCGメモリ増設(改造)の有無を表示します(ファイル名と表示文はてきとうです)。

調査のためPCGメモリにアクセスしますが、画面モードが解像度768×512の場合はPCGメモリに
アクセスできないため一時的に画面モードを変更します。

XEiJ 0.26.02.08以降では「フルパターンメモリ」を有効にすると増設ありとして認識されます
(メニューから設定を変更した後はリセットが必要です)。

参考: [X68000のCYNTHIA(スプライト用チップ)の真の性能がついに実証される - posfie](https://posfie.com/@rentan_org/p/xXPOpN2)
