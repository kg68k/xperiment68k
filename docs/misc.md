...
## e_ver232c
`ERS E_VER232C`でRSDRV.SYSのバージョンを取得して表示します。  
RSDRV.SYSを組み込んでおく必要があります
(ただし、version 1はバージョン取得機能が未対応のため`$ffff_ffff`になります)。


## hasophash
HAS060X.Xの内部で使用されている命令(疑似命令含む)のハッシュ値を、同じアルゴリズムで計算します。  
コマンドライン引数を指定したらその文字列のハッシュ値を表示します。
省略した場合は、すべての命令のハッシュ値を表示します。


## is31days
月の日数が31日まであるか確認するコードです。
[戀塚昭彦氏によるZ80用のコード](https://x.com/koizuka/status/1994315585937690912)
の移植版です。
...
## nminoreset
NMIスイッチの割り込み処理ルーチンを差し替えて常駐終了します。

差し替えたルーチンはIOCS ROMの処理とほぼ同じですが、
システムポートによるNMIリセット処理が削除されています。
このためNMIスイッチを押すと割り込みが発生し続け、ハングアップします。
...
## s_level
`SCSI _S_LEVEL`でSCSI IOCSのバージョンを取得して表示します。 
SCSI IOCSを組み込んでおく必要があります。
