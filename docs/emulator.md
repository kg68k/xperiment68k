...
XM6 TypeGはversion 3.38で実機と同等の挙動となったため実機として判定されます。


## lsrb_dxdy
`lsr.b d3,d4`命令を実行し、結果が想定と異なる場合にその値を表示します。
すべて正しければ何も表示されません。

より厳密な検証にはレジスタに入れておく値をより広範囲にする必要がありますが、
現状では一部の値の範囲のみ実行しています。

実機で動作確認していないので、想定そのものが間違っているかもしれません。
