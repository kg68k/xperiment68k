...
## keep_undersized
メモリ管理ポインタとPSPだけのサイズで常駐するだけのプログラムで、常駐プロセスの常駐検査を行う
コードの動作テスト用です。

なにかコマンドライン引数を指定するとメモリ管理ポインタのみのサイズで常駐します。
これは`DOS _KEEPPR`の引数に負数を指定することで実現していますが、不正な指定なので
その後の動作に支障が出ます。
...
## super_exec
スーパーバイザモードに切り替えてからスタックに0を積み、
コマンドライン引数で指定したコマンドを`DOS _EXEC`で実行し結果を表示します。
コマンド終了後に0を積んだスタックの内容を表示します。

独自のスタックを用意していないため、スーパーバイザモードに切り替え後も
起動時のスタックがそのまま使われます。
