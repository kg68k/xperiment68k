...
ProgramStart:
  ...
  @@:
    ...
    move d0,d2  ;チャンネル番号
    swap d2
    move #O3_VOLFLT_INQUIRY,d2  ;省略時は設定取得
    SKIP_SPACE a0
    beq @f
      bsr ParseIntWord
      move d0,d2  ;フィルタモード
...
PrintAllChannels:
  ...
  1:
    ...
    move d6,d2  ;チャンネル番号
    swap d2
    move #O3_VOLFLT_INQUIRY,d2
    OPM _M_VOLFLT

    .fail O3_VOLFLT_OFF.ne.0
    cmpi.l #O3_VOLFLT_ON,d0
    bhi @f
      bsr ToDecString
      bra 8f
    @@:
...
strHeader: .dc.b 'Ch: VOL filter mode',CR,LF,0
strColon: .dc.b ': ',0
