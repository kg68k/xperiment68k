...
ProgramStart:
  ...
  SKIP_SPACE a0
  bne @f
    bsr PrintAllChannels  ;引数省略時は全チャンネルの割り当てを表示(OPMDRV3.X専用)
    bra 9f
  @@:
    bsr ParseIntWord
    move d0,d2  ;チャンネル番号
    swap d2
    move #O3_ASSIGN_INQUIRY,d2  ;トラック番号省略時は割り当て取得(OPMDRV3.X専用)
    SKIP_SPACE a0
    beq @f
      bsr ParseIntWord
      move d0,d2  ;トラック番号
...
PrintAllChannels:
  ...
  1:
    ...
    move d6,d2  ;チャンネル番号
    swap d2
    move #O3_ASSIGN_INQUIRY,d2
    OPM _M_ASSIGN

    cmpi.l #O3_TRACK_MIN,d0
    bcs @f
    cmpi.l #O3_TRACK_MAX,d0
    bhi @f
      bsr ToDecString
      bra 8f
    @@:
...
strHeader: .dc.b 'Ch: Track',CR,LF,0
strColon: .dc.b ': ',0
