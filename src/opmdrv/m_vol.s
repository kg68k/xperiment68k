...
ProgramStart:
  ...
  SKIP_SPACE a0
  bne @f
    bsr PrintAllChannels  ;引数省略時は全チャンネルのボリュームを表示
    bra 9f
  @@:
    bsr ParseIntWord
    move d0,d2  ;チャンネル番号
    swap d2
    move #O3_VOL_INQUIRY,d2  ;ボリューム省略時は設定取得
    SKIP_SPACE a0
    beq @f
      bsr ParseIntWord
      move d0,d2  ;ボリューム
...
PrintAllChannels:
  ...
  1:
    ...
    move d6,d2  ;チャンネル番号
    swap d2
    move #O3_VOL_INQUIRY,d2
    OPM _M_VOL

    .fail O3_VOL_MIN.ne.0
    cmpi.l #O3_VOL_MAX,d0
    bhi @f
      bsr ToDecString
      bra 8f
    @@:
...
strHeader: .dc.b 'Ch: Volume (0-127)',CR,LF,0
strColon: .dc.b ': ',0
